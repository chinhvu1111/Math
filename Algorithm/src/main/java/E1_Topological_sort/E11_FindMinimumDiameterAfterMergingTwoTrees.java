...
public class E11_FindMinimumDiameterAfterMergingTwoTrees {
...
//        System.out.printf("Node:%s, left:%s, right:%s\n", node, firstVal, secondVal);
        rs[0]=Math.max(rs[0], firstVal+secondVal);
        return firstVal+1;
    }
...
//        }
        ...
        //Time: O(V1+E1)
        maxLenLeft[0]= Math.max(getLongestPath(adjNodes1, rootLeft, -1, maxLenLeft), maxLenLeft[0]);
        //Time: O(V2+E2)
        maxLenRight[0] = Math.max(getLongestPath(adjNodes2, rootRight, -1, maxLenRight), maxLenRight[0]);
...
//                edges2 = {};
        ...
        //
        //** CT Mở rộng (E141_FindMinimumDiameterAfterMergingTwoTrees):
