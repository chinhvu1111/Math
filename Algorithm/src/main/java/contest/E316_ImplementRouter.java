...
public class E316_ImplementRouter {

    static class Router {
        public static Queue<int[]> packets;
        public static HashMap<Integer, List<int[]>> cache;
        ...
        public static void removeElement(List<int[]> list, int[] removedElement){
            int low=0, high=list.size()-1;
            int rs=-1;
            while (low<=high){
                int mid=low+(high-low)/2;
                if(list.get(mid)[2]<=removedElement[2]){
                    rs=mid;
                    high=mid-1;
                }else{
                    low=mid+1;
        ...
        //start,x,...,y, end
        //Find(x)
        public static int searchUpperBound(List<int[]> list, int key){
            int low=0, high=list.size()-1;
            int rs=-1;
            while (low<=high){
                int mid=low+(high-low)/2;
                if(list.get(mid)[2]>=key){
                    rs=mid;
                    high=mid-1;
                }else{
                    low=mid+1;
        ...
        public static int searchLowerBound(List<int[]> list, int key){
            int low=0, high=list.size()-1;
            int rs=-1;
            while (low<=high){
                int mid=low+(high-low)/2;
        ...
        public boolean addPacket(int source, int destination, int timestamp) {
            ...
            if(packets.size()==mem&&!packets.isEmpty()){
                ...
                List<int[]> curList = cache.getOrDefault(removedPacket[1], new ArrayList<>());
                //Sort ==> We can remove by searching
//                curList.remove(removedPacket);
                removeElement(curList, removedPacket);
                cache.put(removedPacket[1], curList);
            }
            ...
            List<int[]> curList = cache.getOrDefault(destination, new ArrayList<>());
            curList.add(newPacket);
            cache.put(destination, curList);
            return true;
        }
        ...
        public int getCount(int destination, int startTime, int endTime) {
            ...
            List<int[]> curList = cache.get(destination);
//            int count=0;
...
//            }
//            return count;
            int low = searchUpperBound(curList, startTime);
            int high = searchLowerBound(curList, endTime);
